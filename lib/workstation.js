...
(function(ns) {
	...
	function newWidget(args) {
		...
		widget.addWidget = function(type, args) {
			guard(arguments.length === 2, "Expects two arguments: type and args (the rest), had " + arguments.length + " argument");
			guard(!(type === "screen" && this.type === "screen"), "Unable to add screen to screen.");
...
function label(args) {
	...
	workstation.ast().lastScreen().addWidget("label", args);
}

function textbox(args) {
	workstation.ast().lastScreen().addWidget("textbox", args);
}

function passwordfield(args) {
	workstation.ast().lastScreen().addWidget("passwordfield", args);
}

function button(args) {
	workstation.ast().lastScreen().addWidget("button", args);
}
...
function table(args, block) {
	...
	workstation.ast().lastScreen().addWidget("table", args);
...
function row(args) {
	workstation.ast().lastScreen().lastWidget().addWidget("row", args);
}
